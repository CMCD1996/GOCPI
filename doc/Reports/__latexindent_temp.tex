...
GOCPI adopted Data Science best practices, as described by Wilson et al \cite{J:10}. Although these practices are
...
continuously Google Drive \cite{Google_Drive}. Git \cite{Git} was used to manage version control for GOCPI's source code, data, documentation and results.
...
who have access to the repository. Each contributor can create custom copies on branches through pull requests on the master branch. Contributors
...
approving suitable push requests to integrate changes to the master branch. Collaborative efforts were possible with
commit messages describing the contributions from each contributor. This project had one developer. Git ensures the histories of code, work and authors are stored.
...
The \textbf{src} subdirectory stores the source code for preparing raw data, partitioning sets of geographies with different granularities and the
...
The intention is to empower users to discuss energy investment and policy decisions made by public and private parties.
Additionally, GOCPI intends to reduce the discrepancy in education between energy policies and help assess the feasibility of meeting the International Energy Agency's Sustainable Development Scenario \cite{IEA_WEM}.
...
Python has many useful packages such as NumPy, Scikit-learn, os, csv and Pandas useful for creating the GOCPI package. Programming is quick due to Python's dynamic nature.
...
which are less well-maintained. Anaconda allows you to create custom environments which was necessary for creating scalable linear optimization problems to express energy systems \textbf{Insert Reference}.
Pip is Python's default package manager and included within the Anaconda package. 
Pip was used to install and update packages for python not available on Anaconda such as twine and the custom GOCPI package developed for this project \textbf{[Insert Reference]}.
...
The \textbf{GOCPI OseMOSYS Structure.xlsx} file describes the sets, paramaters, constraints and objective function of an energy systems model.
User may toggle statements to include. 
...
Excel is best for two dimensional variables or informatino stored Codd-Boyce relational databases \cite{CBNF}.
Many of the parameters in energy system modelling are three or more dimensional. 
...
The python packages are \textbf{cplex} and \textbf{docplex}. The cplex package contains classes for accessing CPLEX from the Python programming language. The Cplex class is the most important class in this module as provides methods for creating, modifying, querying, or solving an optimisation problem.
...
The user can use docplex to solve the linear programme on a local solver. 
...
Accessibility issues to commercial solvers will need to be addressed to enable the distribution of the GOCPI product.
...
Docstrings and commenting are the most important

\subsubsection{Infrastructure}
GOCPI creates scalable energy systems optimisation models which change in complexity depending on there size.
Computations either took place locally on a 128 GB, four core Apple MacBook Pro or remotely on a cloud service. 
...
The combined use of both the OseMOSYS methodology and CPLEX solver required the formulation of an lp-format file through combining user-defined data and model text files.
...
The OseMOSYS model file is stored in an excel sheet to make the OseMOSYS methodology transparent for the user. The model structure is converted into the required text file using the CreateModelFile in the GOCPI package.
